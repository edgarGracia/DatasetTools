...
def read_annotation(path: Path, is_xyxy: bool = False,
    separator: str = " ", label_mapping: Union[list, None] = None
    ) -> List[dict]:
...
def plot_bb(images_path: Path, annotations_path: Path,
    ...
    for img_path in tqdm(list(images_path.iterdir())):
        ...
        # Parse annotation
        annot_path = annotations_path.joinpath(img_path.name + ".txt")
        if not annot_path.exists():
            print(f"{annot_path} not exist!!")
            continue
        
        bbs = read_annotation(annot_path, is_xyxy, separator, label_mapping)
        ...
        for i, bb in enumerate(bbs):
            ...
            if show_label and show_score:
                label = f"{bb['label']}: {round(bb['score'], 3)}"
            elif show_label:
                label = bb["label"]
            elif show_score:
...
if __name__ == "__main__":
    ...
    color = (int(i) for i in args.color) if args.color is not None else None
